...
public class UtilCorrectivoReporteMobileOperador extends UtilVisitaReporteOperador {
	...
	public static void addChequeo(SeccionReporteData seccionMantenimiento, Preventivo preventivo) {
		...
		// Tipo de activo = 6 para los chequeos de activo generico
		...
		if (itemsChequeados != null){
			...
			for (ItemChequeado icd : itemsChequeados){
				if (icd.getValor() != null && icd.getValor().equalsIgnoreCase("R")){
					if(primerItem) {
						detalle.setValor(icd.getItemChequeo().getTexto());
	...
	public static void addPendientes(SeccionReporteData seccionMantenimiento, Preventivo preventivo) {
		...
		addPendientes(seccionMantenimiento, preventivo.getChequeo(), "");

		if (preventivo.getActivo().getTipo() == 1){
			ChequeoSurtidor cs = new ChequeoSurtidorDao().get(chequeo.getId());
			for (Chequeo chequeoPico : cs.getListaDeChequeosPicos()){
				addPendientes(seccionMantenimiento, chequeoPico, "Nro: " + ((ChequeoPico)chequeoPico).getPico().getNumeroEnLaEstacion());	
			}

			for (Chequeo chequeoProducto : cs.getListaDeProductos()){
				addPendientes(seccionMantenimiento, chequeoProducto, ((ChequeoProducto)chequeoProducto).getProducto().getNombre());
			}
		}
	}


	private static boolean addPendientes(SeccionReporteData seccionMantenimiento, Chequeo chequeo, String texto) {
			...
			for(Pendiente p : ic.getListaDePendientes()) {
				...
				if (p.isComentarioVisible()){
					if (agregarTitulo){
						...
						/*detalleheader = new DetalleReporteData();
						detalleheader.setLabel("Comentario");
						detalleheader.setValor("Plazo");
						detalleheader.setFormato(DetalleReporteData.COMPLETA);
						seccionMantenimiento.getDetalles().add(detalleheader);*/
