...
public class IUDialogoAgyModMarcaTipoActivoGenerico extends DialogBox{
	...
	public IUDialogoAgyModMarcaTipoActivoGenerico(boolean autoHide, IUGestionarMarcaTipoActivoGenerico padre, GlassPopup glass, PopupCargando popUp, final MarcaActivoGenerico seleccionado) {
		super(autoHide);
		this.glass = glass;
		this.popUp = popUp;
		this.padre = padre;
	...
	private void agregar() {
		if (nombreEsValido()){
			popUp.show();
			MarcaActivoGenerico item = new MarcaActivoGenerico();
			item.setNombre(txtNombre.getText());
			item.setTipo(new TipoActivoGenerico(padre.getTipoActivoSeleccionado().getId()));

			ProyectoBilpa.greetingService.agregarMarcaActivoGenerico(item, new AsyncCallback<Boolean>() {
	...
	private void editar() {
		if (nombreEsValido()){
			...
			ProyectoBilpa.greetingService.modificarMarcaActivoGenerico(seleccionado, new AsyncCallback<Boolean>() {
				public void onFailure(Throwable caught) {
					error();
				}
	...
	private void success(Boolean result) {
		...
		if (result){
			...
			ValidadorPopup vpu = new ValidadorPopup(new GlassPopup(), "Info", "Ya existe una marca con el nombre: " + txtNombre.getText());
			vpu.showPopUp();
		}
	}
}
