...
public class IUDialogoAgyModTipoActivoGenerico extends DialogBox{
	...
	public IUDialogoAgyModTipoActivoGenerico(boolean autoHide, IUGestionarTipoActivoGenerico padre, GlassPopup glass, PopupCargando popUp, final TipoActivoGenericoData tipoActivoGenericoSeleccionado) {
		super(autoHide);
		this.glass = glass;
		this.popUp = popUp;
		this.padre = padre;
		...
		if (tipoActivoGenericoSeleccionado != null){
			txtNombre.setText(tipoActivoGenericoSeleccionado.getNombre());
		}
		...
		txtNombre.addKeyPressHandler(new KeyPressHandler() {
			public void onKeyPress(KeyPressEvent event) {
				...
				{
					sumbit(tipoActivoGenericoSeleccionado);
				}
			}
		});
	...
	private void sumbit(final TipoActivoGenericoData tipoActivoGenericoSeleccionado) {
		if (tipoActivoGenericoSeleccionado == null){
			agregarTipoActivoGenerico();
		} else {
			editarTipoActivoGenerico();
		}
	}
	...
	private void agregarTipoActivoGenerico() {
		if (nombreEsValido()){
			popUp.show();
			TipoActivoGenerico tipoActivoGenerico = new TipoActivoGenerico();
			tipoActivoGenerico.setNombre(txtNombre.getText());

			ProyectoBilpa.greetingService.agregarTipoActivoGenerico(tipoActivoGenerico, new AsyncCallback<Boolean>() {
	...
	private void editarTipoActivoGenerico() {
		if (nombreEsValido()){
			popUp.show();
			tipoActivoGenericoSeleccionado.setNombre(txtNombre.getText());

			ProyectoBilpa.greetingService.modificarTipoActivoGenerico(tipoActivoGenericoSeleccionado, new AsyncCallback<Boolean>() {
				public void onFailure(Throwable caught) {
					error();
				}
	...
	private void success(Boolean result) {
		if (result){
			...
			ValidadorPopup vpu = new ValidadorPopup(new GlassPopup(), "Info", "Ya existe un tipo de activo con el nombre: " + txtNombre.getText());
			vpu.showPopUp();
			popUp.hide();
		}
	}
